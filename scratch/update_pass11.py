...
<head>
  ...
  <style>
    /* ========================================================================= */
    /* 1. UNIFIED DESIGN SYSTEM — CSS CUSTOM PROPERTIES (Pass 11 Light-First)    */
    /* ========================================================================= */
...
<body class="antialiased selection:bg-[#0FA7A0] selection:text-white">
  ...
  <header class="sticky top-0 z-40 backdrop-blur-md border-b shadow-sm transition-all h-16" style="background: var(--bg-section); border-color: var(--border-main);">
    <div class="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2 sm:gap-2.5">
        ...
        <!-- TRY NOW Button (Pass 11 Dedicated Login Route) -->
        <button onclick="handleTryNowClick()" id="btn-nav-try-now" class="h-9 px-3.5 rounded-xl font-bold text-xs shadow-sm hover:opacity-95 transition-all flex items-center gap-1.5 cta-press whitespace-nowrap text-white" style="background: linear-gradient(135deg, var(--nobi-teal), var(--nobi-accent));" aria-label="Try Nobi Digital Caretaker">
          <span>TRY NOW</span>
        ...
        <button onclick="cycleThemeMode()" id="btn-nav-theme" class="h-9 w-9 rounded-xl flex items-center justify-center transition-all cta-press" style="background: var(--bg-card); border: 1px solid var(--border-main); color: var(--text-primary);" title="Toggle Theme (Light / Dark / System)" aria-label="Toggle Color Theme">
        ...
        <!-- Notification Bell -->
        <button onclick="openNotificationsDrawer()" class="relative h-9 w-9 rounded-xl flex items-center justify-center transition-all cta-press flex-shrink-0" style="background: var(--bg-card); border: 1px solid var(--border-main); color: var(--text-primary);" aria-label="Open Caretaker Notifications">
        ...
        <!-- Profile Control Center Dropdown Trigger -->
        <div class="relative" id="profile-control-center-wrapper">
          <button onclick="toggleProfileDropdown()" id="btn-nav-profile" class="h-9 pl-1.5 pr-2.5 rounded-xl flex items-center gap-2 transition-all cta-press shadow-sm focus:outline-none" style="background: var(--bg-card); border: 1px solid var(--border-main); color: var(--text-primary);" aria-expanded="false" aria-haspopup="true" aria-label="User Account Menu">
          ...
          <div id="nav-profile-dropdown" class="hidden absolute right-0 top-full mt-2 w-80 rounded-2xl shadow-2xl z-50 overflow-hidden text-left p-1.5" style="background: var(--bg-card); border: 1px solid var(--border-main);">
              ...
              <div class="flex items-center gap-3">
                ...
                <div class="space-y-0.5 min-w-0 flex-1">
                  <div class="flex items-center justify-between gap-1">
                    <span id="dropdown-user-name" class="font-bold text-xs font-display truncate" style="color: var(--text-primary);">Vishal Khadatare</span>
                  ...
                  <span id="dropdown-user-email" class="text-[11px] truncate block" style="color: var(--text-secondary);">vishal.k@gmail.com</span>
                </div>
              </div>
            ...
            <div class="mt-1 pt-1 border-t" style="border-color: var(--border-subtle);">
              <button onclick="closeProfileDropdown(); handleAuthActionClick();" id="dropdown-btn-auth-action" class="w-full px-3 py-2 rounded-lg hover:bg-rose-500/15 text-rose-500 hover:text-rose-600 transition-colors flex items-center gap-2.5 text-xs font-semibold text-left">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 items-center">
          ...
          <div class="lg:col-span-7 space-y-4 text-left">
            ...
            <div class="pt-2 flex flex-wrap items-center gap-3">
              <!-- Pass 11: Dedicated Try Live Demo Action -->
    ...
    <div class="relative max-w-xl w-full rounded-2xl shadow-2xl p-5 sm:p-6 overflow-y-auto max-h-[90vh] z-10 space-y-4" style="background: var(--bg-card); border: 1px solid var(--border-main);">
      ...
      <div class="p-4 rounded-xl flex items-center gap-3.5" style="background: var(--bg-inner); border: 1px solid var(--border-subtle);">
        ...
        <div class="space-y-0.5">
          <div class="flex items-center gap-2">
            <span id="profile-modal-name" class="text-sm font-bold font-display" style="color: var(--text-primary);">Vishal Khadatare</span>
          ...
          <span id="profile-modal-email" class="text-xs block" style="color: var(--text-secondary);">vishal.k@gmail.com</span>
  ...
  <!-- ========================================================================= -->
  <!-- JAVASCRIPT ARCHITECTURE ENGINE (Pass 11 Complete)                         -->
  <!-- ========================================================================= -->
  <script>
    ...
    // 4. THEME CONTROLLER (Pass 11 Light-First)
    ...
    // 5. WEB AUDIO API CHIME (Pass 11 Voice Mic Activation Sound)
    ...
    function toggleVoiceRecognition() {
      ...
      // Pass 11: Play soft synthesized Web Audio API Chime on activation
    ...
    // 10. PASS 11 DEDICATED LOGIN & NAVIGATION ROUTING
    function handleTryNowClick() {
      const auth = NobiAgentStore.state.auth;
      if (auth && auth.isAuthenticated) {
        scrollToLiveDemo();
      } else {
        window.location.href = 'login.html?returnTo=index.html#live-demo';
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      // Restore Theme (Pass 11: Light default)
...
print(f"[PASS 11 COMPLETE] Compiled {len(html_code)} bytes to {index_path} and synced build scripts.")
